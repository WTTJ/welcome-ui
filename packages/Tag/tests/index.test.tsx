...
describe('<Tag>', () => {
  it('should render correctly', () => {
    ...
    expect(tag).toHaveStyleRule('background-color', theme.colors['nude-30'])
    expect(tag).toHaveStyleRule('color', theme.colors['nude-80'])
  ...
  it('should have correct color for sub variant', () => {
    const { getByTestId } = render(
      <Tag dataTestId="tag" variant="secondary-blue">
        {content}
      </Tag>
    )
    const tag = getByTestId('tag')

    expect(tag).toHaveStyleRule('background-color', theme.colors['secondary-blue'])
  })
