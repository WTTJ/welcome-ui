...
Welcome UI V5 is focus on rework of our **foundations** and move from reakit to **ariakit**.
...
We rework our theme on the colors, spacing, typography... properties. Use the migration script to migrate easily to V5 on welcome-ui project.
...
We also migrate to the new **[Ariakit](https://ariakit.org)** library _(reakit v2)_ on some of our components. To migrate easely you can find components migration below.

To resume:

- now we pass on a `store` property the state of component and remove State from hook name.
...
- `visible` is now replace by `open` property
- force open a component now is with `defaultOpen` property
...
- access to the actual state of component with `useState`:
...
We changed our colors dark and light to be full compatible with the coming dark mode (in beta). Now dark are black rgba with transparency, and light are white rgba with transparency.
...
We migrate to Ariakit and add an `useAccordion` hook to play with the component store.
...
We migrate to Ariakit, now we pass the component state from `useDrawer` on `store` property:
...
and `defaultTabState` become ` defaultTabStore`
...
- We migrate to Ariakit and add an `useModal` hook to play with the component store.
- `onClose` has been changed from an optionnal prop of **Modal** to an optionnal option you can pass to useModal
...
- We migrate to Ariakit and add an `usePopover` hook to play with the component store.
...
We removed the check icon on radio input and change the style of it. We also rework accesibility.

### Tabs

We migrate to Ariakit, now we pass the component state from `useTab` on `store` property:
