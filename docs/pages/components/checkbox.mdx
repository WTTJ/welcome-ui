...
Use 'indeterminate' prop to manage the behaviour of a parent checkbox in response to children checkboxes
...
function() {
  ...
  const [checked, setChecked] = React.useState(false)
  const [checkedChildren, setCheckedChildren] = React.useState([])
