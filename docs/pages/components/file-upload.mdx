...
<Form
  initialValues={{
    file: 'https://doc-example/example.docx?v=63731713698',
  }}
>
...
function() {
  ...
  return (
    <Form initialValues={{ file: 'https://doc-example/example.docx?v=63731713698' }}>
      <Field label="File">
        <FileUpload name="file" preview={Preview}>
          {({ openFile, disabled }) => (
            <Button onClick={openFile} disabled={disabled}>
              Custom preview
            </Button>
...
<Form initialValues={{}}>
  <Field label="File">
    <FileUpload name="file" multiple preview={null}>
