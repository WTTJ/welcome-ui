export { getStaticProps } from '../../getStaticProps'

import {
  component,
  dependencies,
  name,
  peerDependencies,
  version,
} from '@welcome-ui/time-picker/package.json'

<showcase
  version={version}
  name={name}
  component={component}
  description="Allows user to select a predefined time."
/>

## About

It is based on the [react-datepicker](https://github.com/Hacker0x01/react-datepicker) library.

_Note_: Pass a value of `null` if you don't want the default value of `Date.now()`.

## Simple use case

```jsx
function () {
  const [value, setValue] = React.useState(Date.now())

  const handleChange = newValue => {
    setValue(newValue)
  }

  return (
    <Field label="Time">
      <TimePicker icon={<DateIcon />} name="welcome" onChange={handleChange} value={value} />
    </Field>
  )
}
```

## Add an icon on the right

```jsx
function () {
  const [value, setValue] = React.useState(Date.now())

  const handleChange = newValue => {
    setValue(newValue)
  }

  return (
    <Field label="Time">
      <TimePicker icon={<DateIcon />} iconPlacement="right" name="welcome2" onChange={handleChange} value={value} />
    </Field>
  )
}
```

## Pass props to TimePicker

```jsx
function () {
  const [value, setValue] = React.useState(Date.now())

  const handleChange = newValue => {
    setValue(newValue)
  }

  return (
    <Field>
      <TimePicker name="welcome3" popperProps={{ zIndex: 50 }} timeIntervals={30} onChange={handleChange} value={value} />
    </Field>
  )
}
```

## Properties

<props propTypes={props.propTypes.TimePicker} />

## Packages

### Dependencies

<dependencies dependencies={dependencies} />

### Peer dependencies

<dependencies dependencies={peerDependencies} />

<pagination />
