...
Pass `orientation: 'vertical'` when creating the store with `useTab` to display the tabs vertically instead of horizontally, e.g. `const store = useTab({ orientation: 'vertical' })`. Defaults to `'horizontal'`.
