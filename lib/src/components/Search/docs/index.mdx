...
The results list is capped at `155px` by default. Override it with the `--searchMenuMaxHeight` CSS variable, either inline or from any parent:

```jsx
<Search search={handleSearch} style={{ '--searchMenuMaxHeight': '20rem' }} />
```
