---
category: typography
description: The Text component is a fundamental UI element used to display and style text content within an application. It provides various typography options, such as font size, weight, color, and alignment, allowing for consistent and customizable presentation of textual information. The word-break css property is set to break-word by default and can be overwritten using the wordBreak props. This component is essential for conveying information clearly and effectively in the user interface.
packageName: text
title: Text
---

### Overwrite tag

You can pass a tag name with `as` to use that tag type with the styling from another tag. For example…

<div data-playground="overwrite.tsx" data-component="Text"></div>

### Truncation

Set the number of lines you want to display with `lines`. Your text will be displayed truncated with an ellipsis (`...`) at the end (if necessary).

<div data-playground="truncation.tsx" data-component="Text"></div>


### withDash

With the `withDash` property, a dash is added before the text in headings _(h0 to h6)_.

<div data-playground="withDash.tsx" data-component="Text"></div>
