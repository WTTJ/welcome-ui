...
The dropdown list is capped at `155px` by default. Override it with the `--selectMenuMaxHeight` CSS variable, either inline or from any parent:

```jsx
<Select options={options} style={{ '--selectMenuMaxHeight': '20rem' }} />
```
