...
StyleDictionary.registerTransform({
  filter: token => {
    // Exclude radius tokens with value 9999 (handled by size/radiusToInfinity)
    if (token.name.includes('radius') && token.$value === 9999) return false
...
StyleDictionary.registerTransform({
  filter: token => token.name.includes('radius') && token.$value === 9999,
...
StyleDictionary.registerFormat({
  format: function ({ dictionary, options }) {
    ...
    const sortedTokens = [...dictionary.allTokens].sort((a, b) => a.name.localeCompare(b.name))
