...
const twoXlRegex = new RegExp(/-2xl/g)
const twoXsRegex = new RegExp(/-2xs/g)

StyleDictionary.registerTransform({
  filter: token => token.value.match(zeroValueRegex),
...
StyleDictionary.registerTransform({
  filter: token => token.name.match(twoXlRegex),
  name: 'name/2xlToXxl',
  transform: token =>
    `${token.name.replaceAll(twoXlRegex, '-xxl')}: ${token.value};\n  --${token.name}`,
  type: 'name',
})

StyleDictionary.registerTransform({
  filter: token => token.name.match(twoXsRegex),
  name: 'name/2xsToXxs',
  transform: token =>
    `${token.name.replaceAll(twoXlRegex, '-xxs')}: ${token.value};\n  --${token.name}`,
  type: 'name',
})

StyleDictionary.registerTransform({
  filter: token => token.name.startsWith('border-radius-'),
  name: 'name/borderRadiusToRadius',
  transform: token =>
    `${token.name.replace('border-radius-', 'radius-')}: ${token.value};\n  --${token.name}`,
  type: 'name',
})
