...
Use `hint`, `warning`, `danger` or `success` properties on Field component to add a variant status on your checkbox. The label, hint or border color are set by magic 🪄
...
function() {
  const FormChildren = ({ control, register }) => (
    <FieldGroup label="Check the frameworks you have already worked with" hideLabel>
      <Controller
        name="react"
        control={control}
