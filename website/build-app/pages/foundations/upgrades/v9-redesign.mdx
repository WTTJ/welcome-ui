...
- Replace `Modal.Header title="..."` with the new composition API using children as shown above.
- If you have code using `Modal.Body as={Window.Body}` and you relied on automatic modal spacing, update the consuming `Window.Body` to accept and merge `className` so both modal and window styles are preserved.
- Where you want the modal to go fullscreen, use the `fullscreen` prop or toggle a boolean state passed to the Modal.
