...
The shape variants no longer exist. All the buttons have the same border-radius now.
The `xs` size has been removed, `lg` is the new default size. New sizes are `sm`, `md` and `lg`.
