---
name: Button
route: /components/button
menu: Components
---

import { Props, Link } from 'docz'
import { Playground } from '../../../docz/'

import { Badge } from '../Badge'
import { Box } from '../Box'
import { Button } from './'
import { Icon } from '../Icon'

# Button

## Variants

Admire these beautiful variants 🌈

<Playground>
  <Box width="100%" fontSize="body3">
    Defaults
  </Box>
  <Button>primary (default)</Button>
  <Button variant="primary-reverse">primary reverse</Button>
  <Button variant="secondary">secondary</Button>
  <Button variant="secondary-reverse">secondary reverse</Button>
  <Button variant="tertiary">tertiary</Button>
  <Button variant="tertiary-reverse">tertiary reverse</Button>
  <Box width="100%" fontSize="body3" marginTop="10px">
    States
  </Box>
  <Button variant="warning">warning</Button>
  <Button variant="warning-reverse">warning reverse</Button>
  <Button variant="danger">danger</Button>
  <Button variant="danger-reverse">danger reverse</Button>
</Playground>

## Formats

Use size property with `xs` `sm` `md`(default) or `lg`.

<Playground>
  <Button size="xs">Tiny</Button>
  <Button size="sm">Small</Button>
  <Button>Medium</Button>
  <Button size="lg">Large</Button>
</Playground>

## Disabled

All disabled buttons have the same style.

<Playground>
  <Button variant="primary-danger" disabled>
    Disabled
  </Button>
</Playground>

## With icons

<Playground>
  <Button>
    <Icon name="language" size="sm" />
    <span>Button</span>
  </Button>
  <Button variant="secondary">
    <Icon name="language" size="sm" />
    <span>Button</span>
  </Button>
  <Button variant="tertiary">
    <Icon name="language" size="sm" />
    <span>Button</span>
  </Button>
  <Button disabled>
    <Icon name="language" size="sm" />
    <span>Button</span>
  </Button>
</Playground>

## Shape

Get same size on `width` and `height`, can have a 'circle' or 'square' `shape`.

<Playground flex>
  <Button shape="square" size="xs">
    <Icon name="language" size="xs" />
  </Button>
  <Button shape="circle" size="xs">
    <Icon name="language" size="xs" />
  </Button>
  <Button shape="square" size="sm">
    <Icon name="language" size="sm" />
  </Button>
  <Button shape="circle" size="sm">
    <Icon name="language" size="sm" />
  </Button>
  <Button shape="square">
    <Icon name="language" size="sm" />
  </Button>
  <Button shape="circle">
    <Icon name="language" size="sm" />
  </Button>
  <Button shape="square" size="lg">
    <Icon name="language" size="md" />
  </Button>
  <Button shape="circle" size="lg">
    <Icon name="language" size="md" />
  </Button>
</Playground>

## Transform to a link

You can transform your Button component with `as` property to add you linker, `href` or `to`... all that you want.

<Playground flex>
  <Button as="a" href="http://www.welcometothejungle.co" target="_blank" shape="square">
    <Icon name="external_link" size="sm" />
  </Button>
  <Button
    as="a"
    href="http://www.welcometothejungle.co"
    target="_blank"
    rel="noopener nofollow"
    variant="secondary"
  >
    <Icon name="sun" size="sm" />
    <span>Welcome</span>
  </Button>
  <Button as={Link} to="/" variant="tertiary">
    Homepage
  </Button>
</Playground>

## Width

You can add `width` property from [xstyled](https://www.smooth-code.com/open-source/xstyled/docs/system-props/#layout).

<Playground>
  <Button width={1}>full width</Button>
  <Button width={1 / 2}>half width</Button>
</Playground>

## Radius

You can add `borderRadius` property from [xstyled](https://www.smooth-code.com/open-source/xstyled/docs/system-props/#borders).

<Playground>
  <Button>default</Button>
  <Button borderRadius="10px">10px</Button>
</Playground>

## Properties

<Props of={Button} />
