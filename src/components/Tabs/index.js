...
const Tabs = ({ children, defaultActiveTab, handleActiveTab }) => {
  ...
  const setPositionActiveBar = useCallback(() => {
    const activeBarElm = activeRef && activeRef.current
    const listElm = listRef && listRef.current

    const left = activeBarElm.getBoundingClientRect().left - listElm.getBoundingClientRect().left
    const width = activeBarElm.getBoundingClientRect().width
  ...
  const updateActiveTab = name => {
    // handleActiveTab from property, return to parent tab selected
